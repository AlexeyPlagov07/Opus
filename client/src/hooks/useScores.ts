...
function mapScore(snapshot: QueryDocumentSnapshot): Score {
  ...
  return {
    ...
    composer: String(data.composer ?? 'Unknown Composer'),
    instrument: String(data.instrument ?? 'Piano'),
    difficulty: typeof data.difficulty === 'number' && Number.isFinite(data.difficulty) ? data.difficulty : 5,
